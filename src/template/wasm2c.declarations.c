...
// (imported or internal). Currently these functions may use the segue
// optimization if allowed. The segue optimization uses x86 segments to point to
// a linear memory. We use this optimization when on x86_64 if the compiler
// supports intrinsics for (rd|wr)(fs|gs)base, supports "address namespaces" for
// accessing pointers and supports memcpy on pointers with custom "address
// namespaces". GCC does not support the memcpy requirement, so this leaves only
// clang for now.
#if WASM_RT_ALLOW_SEGUE && (defined(__x86_64__) || defined(_M_X64)) && __clang__
// Test for just one of the segment functions for simplicity
#if __has_builtin(__builtin_ia32_wrgsbase64)
...
#define DEFINE_SINGLE_MEMORY_LOAD(name, t1, t2, t3, force_read) \
  static inline t3 name(wasm_rt_memory_t* mem, u64 addr) {      \
    MEMCHECK(mem, addr, t1);                                    \
    t1 result;                                                  \
    uint8_t WASM_RT_SEGUE_TYPE* addr_ptr =                      \
        (uint8_t WASM_RT_SEGUE_TYPE*)(uintptr_t)addr;           \
    wasm_rt_memcpy(&result, addr_ptr, sizeof(t1));              \
    force_read(result);                                         \
    return (t3)(t2)result;                                      \
  }
...
#define DEFINE_SINGLE_MEMORY_STORE(name, t1, t2)                       \
  static inline void name(wasm_rt_memory_t* mem, u64 addr, t2 value) { \
    ...
    uint8_t WASM_RT_SEGUE_TYPE* addr_ptr =                             \
        (uint8_t WASM_RT_SEGUE_TYPE*)(uintptr_t)addr;                  \
    wasm_rt_memcpy(addr_ptr, &wrapped, sizeof(t1));                    \
  }

#else
#define DEFINE_SINGLE_MEMORY_LOAD DEFINE_LOAD
#define DEFINE_SINGLE_MEMORY_STORE DEFINE_STORE
#endif

// clang-format off
DEFINE_SINGLE_MEMORY_LOAD(i32_load_single_memory, u32, u32, u32, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(i64_load_single_memory, u64, u64, u64, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(f32_load_single_memory, f32, f32, f32, FORCE_READ_FLOAT)
DEFINE_SINGLE_MEMORY_LOAD(f64_load_single_memory, f64, f64, f64, FORCE_READ_FLOAT)
DEFINE_SINGLE_MEMORY_LOAD(i32_load8_s_single_memory, s8, s32, u32, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(i64_load8_s_single_memory, s8, s64, u64, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(i32_load8_u_single_memory, u8, u32, u32, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(i64_load8_u_single_memory, u8, u64, u64, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(i32_load16_s_single_memory, s16, s32, u32, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(i64_load16_s_single_memory, s16, s64, u64, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(i32_load16_u_single_memory, u16, u32, u32, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(i64_load16_u_single_memory, u16, u64, u64, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(i64_load32_s_single_memory, s32, s64, u64, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_LOAD(i64_load32_u_single_memory, u32, u64, u64, FORCE_READ_INT)
DEFINE_SINGLE_MEMORY_STORE(i32_store_single_memory, u32, u32)
DEFINE_SINGLE_MEMORY_STORE(i64_store_single_memory, u64, u64)
DEFINE_SINGLE_MEMORY_STORE(f32_store_single_memory, f32, f32)
DEFINE_SINGLE_MEMORY_STORE(f64_store_single_memory, f64, f64)
DEFINE_SINGLE_MEMORY_STORE(i32_store8_single_memory, u8, u32)
DEFINE_SINGLE_MEMORY_STORE(i32_store16_single_memory, u16, u32)
DEFINE_SINGLE_MEMORY_STORE(i64_store8_single_memory, u8, u64)
DEFINE_SINGLE_MEMORY_STORE(i64_store16_single_memory, u16, u64)
DEFINE_SINGLE_MEMORY_STORE(i64_store32_single_memory, u32, u64)
// clang-format on
