...
def Compile(cc, c_filename, out_dir, use_c11, *cflags):
    ...
    if IS_WINDOWS:
        ...
        cstd_flag = ['-std=c11'] if use_c11 else ['-std=c99']
        args += cstd_flag + ['-c', c_filename, '-o', o_filename, '-O2',
    ...
    with utils.TempDirectory(options.out_dir, 'run-spec-wasm2c-') as out_dir:
        ...
        if options.compile:
            ...
            source_files = [
                ...
                os.path.join(options.wasmrt_dir, 'prebuilt', 'wasm-rt-mem-impl.c'),
            ]

            if options.enable_threads:
                source_files.append(os.path.join(options.wasmrt_dir, 'prebuilt', 'wasm-rt-mem-shared-impl.c'))
