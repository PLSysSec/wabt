...
#define ASSERT_RETURN_MULTI_T(thread_state, type, fmt, f, compare, expected, \
                              found)                                         \
  do {                                                                       \
    g_tests_run++;                                                           \
    if (wasm_rt_impl_try(thread_state) != 0) {                               \
...
#define ASSERT_RETURN_I32(thread_state, f, expected) \
  ASSERT_RETURN_T(thread_state, u32, "u", f, expected)
#define ASSERT_RETURN_I64(thread_state, f, expected) \
  ASSERT_RETURN_T(thread_state, u64, PRIu64, f, expected)
#define ASSERT_RETURN_F32(thread_state, f, expected) \
  ASSERT_RETURN_T(thread_state, f32, ".9g", f, expected)
#define ASSERT_RETURN_F64(thread_state, f, expected) \
  ASSERT_RETURN_T(thread_state, f64, ".17g", f, expected)
