...
#define WASM_RT_MEM_OPS_TEMPLATE_REPLACE
...
static uint64_t get_alloc_size_for_mmap(
    wasm_rt_TEMPLATE_REPLACE_memory_t* memory) {
  assert(!memory->is64 &&
...
void wasm_rt_allocate_memory_TEMPLATE_REPLACE(
    wasm_rt_TEMPLATE_REPLACE_memory_t* memory,
...
static uint64_t grow_memory_impl(wasm_rt_TEMPLATE_REPLACE_memory_t* memory,
                                 uint64_t delta) {
...
uint64_t wasm_rt_grow_memory_TEMPLATE_REPLACE(
    wasm_rt_TEMPLATE_REPLACE_memory_t* memory,
    uint64_t delta) {
...
void wasm_rt_free_memory_TEMPLATE_REPLACE(
    wasm_rt_TEMPLATE_REPLACE_memory_t* memory) {
#if WASM_RT_USE_MMAP
  const uint64_t mmap_size = get_alloc_size_for_mmap(memory);
...
#undef WASM_RT_MEM_OPS_TEMPLATE_REPLACE
